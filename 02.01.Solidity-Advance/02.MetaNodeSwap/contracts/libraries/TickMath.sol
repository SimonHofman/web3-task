...
library TickMath {
    ...
    function maxUsableTick(int24 tickSpacing) internal pure returns (int24) {
        // 计算最大有效tick值
        // 通过将MAX_TICK除以tickSpacing再乘以tickSpacing，确保结果是tickSpacing的整数倍
        // 使用unchecked避免溢出检查，提高gas效率
        unchecked {
    ...
    /// @dev 当 |tick| > 最大 tick 值时抛出异常
    /// @param tick The input tick for the above formula
    /// @param tick 用于上述公式的输入 tick 值
    ...
    function getSqrtPriceAtTick(
        ...
        unchecked {
            ...
            assembly ("memory-safe") {
                // 将tick值进行符号扩展处理，确保其在正确的数值范围内
                // 该函数使用signextend操作码对tick值进行符号位扩展，
                // 第一个参数2表示扩展到32字节（256位）中的第2个字节位置，
                // 保证tick值的符号位正确传播到整个256位整数中。
                tick := signextend(2, tick)
                // mask = 0 if tick >= 0 else -1 (all 1s)
                // 计算掩码：如果tick大于等于0，掩码为0；如果tick小于0，掩码为-1（全1）
                let mask := sar(255, tick)
                // if tick >= 0, |tick| = tick = 0 ^ tick
                // if tick < 0, |tick| = ~~|tick| = ~(-|tick| - 1) = ~(tick - 1) = (-1) ^ (tick - 1)
                // either way, |tick| = mask ^ (tick + mask)
                // 计算tick的绝对值：
                // 当tick >= 0时，|tick| = tick = 0 ^ tick
                // 当tick < 0时，|tick| = ~~|tick| = ~(-|tick| - 1) = ~(tick - 1) = (-1) ^ (tick - 1)
                // 综合两种情况，|tick| = mask ^ (tick + mask)
                absTick := xor(mask, add(mask, tick))
            }

            // 检查tick值是否超出有效范围
            // 如果absTick大于MAX_TICK转换为uint256后的值，则触发InvalidTick错误并回滚交易
