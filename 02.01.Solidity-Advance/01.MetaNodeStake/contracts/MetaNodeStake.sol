    ...
    AccessControlUpgradeable
{
    ...
    function setEndBlock(uint256 _endBlock) public onlyRole(ADMIN_ROLE) {
        require(_endBlock >= startBlock, "end block must be larger than start block");
    ...
    function setMetaNodePerBlock(uint256 _MetaNodePerBlock) public onlyRole(ADMIN_ROLE) {
        require(_MetaNodePerBlock > 0, "invalid parametes");
    ...
    function getMultiplier(
        ...
        require(_to >= _from, "end block must be greater than start block");
        bool success;
    ...
    function pendingMetaNodeByBlockNumber(
        ...
        uint256 accMetaNodePerStake = pool_.accMetaNodePerStake;
        ...
        if (_blockNumber > pool_.lastRewardBlock && stSupply != 0) {
            ...
            accMetaNodePerStake = accMetaNodePerStake + (MetaNodeForPool * (1 ether)) / stSupply;
        }

        return (user_.stAmount * accMetaNodePerStake) / (1 ether) - user_.finishedMetaNode + user_.pendingMetaNode;
    }
    ...
    function updatePool(uint256 _pid) public checkPid(_pid) {
        ...
        if (stSupply > 0) {
            ...
            (bool success3, uint256 accMetaNodePerStake) = pool_.accMetaNodePerStake.tryAdd(totalMetaNode_);
            require(success3, "overflow");
            pool_.accMetaNodePerStake = accMetaNodePerStake;
        }
    ...
    function unstake(
        ...
        // 计算当前块的奖励
        uint256 pendingMetaNode_ = (user_.stAmount * pool_.accMetaNodePerStake) / (1 ether) - user_.finishedMetaNode;
        ...
        user_.finishedMetaNode = (user_.stAmount * pool_.accMetaNodePerStake) / (1 ether);
    ...
    function claim(
        ...
        uint256 pendingMetaNode_ = (user_.stAmount * pool_.accMetaNodePerStake) / (1 ether)
    ...
    function _deposit(uint256 _pid, uint256 _amount) internal {
        ...
        if (user_.stAmount > 0) {
            (bool success1, uint256 accST) = user_.stAmount.tryMul(pool_.accMetaNodePerStake);
            ...
            if (pendingMetaNode_ > 0) {
                ...
                require(success3, "user pendingMetaNode add overflow");
        ...
        if (_amount > 0) {
            ...
            require(success4, "user stAmount add overflow");
        ...
        (bool success6, uint256 finishedMetaNode) = user_.stAmount.tryMul(pool_.accMetaNodePerStake);
        require(success6, "user stAmount mul accMetaNodePerStake overflow");
