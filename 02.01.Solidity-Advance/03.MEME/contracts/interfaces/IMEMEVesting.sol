...
interface IMEMEVesting is IVestingParams {
    ...
    event VestingScheduleRevoked(address indexed token, address indexed beneficiary, uint256 scheduleId, uint256 remainingAmount);

    event EmergencyWithdraw(address indexed token, address indexed beneficiary, uint256 amount);

    function createVestingSchedules(
        address token, address beneficiary, VestingAllocation[] calldata allocations
    ...
    function claimAll(address toekn) external returns (uint256 totalClaimed);
