...
import {IMEMECore} from "./IMEMECore.sol";
...
interface IMEMEHelper {
    ...
    function calculateTokenAmountOutWithFee(
        uint256 bnbIn, IMEMECore.BondingCurveParams memory curve, uint256 feeRate
    ) external pure returns (uint256 tokenOut, uint256 netBNB, uint256 feeBNB);
    ...
    function calculateBNBAmountOutWithFee(
        uint256 tokenIn, IMEMECore.BondingCurveParams memory curve, uint256 feeRate
    ) external pure returns (uint256 netBNB, uint256 feeBNB);
